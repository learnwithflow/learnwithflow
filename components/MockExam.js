...
export default function MockExam({ showPage, showToast }) {
    ...
    const generateMoreQuestions = async (pool, needed, type, chapter) => {
        ...
        try {
            ...
            if (aiQuestions.length < Math.min(10, needed)) {
                setLoading(false);
                showToast?.('⚠️ Failed to generate enough AI questions. Please try again.');
                throw new Error('Not enough questions generated by AI');
            }

            // Shuffle and cap to strictly needed
            const final = shuffleArr([...aiQuestions]).slice(0, needed);
    ...
    const startChapterExam = async (idx) => {
        ...
        try {
            const questions = await generateMoreQuestions([], 30, examType, chapter);
    ...
    const startWeakExam = async () => {
        setLoading(true);
        try {
            const questions = await generateMoreQuestions([], 60, examType, null);
    ...
    const startFullExam = async () => {
        setLoading(true);
        try {
            const questions = await generateMoreQuestions([], 90, examType, 'FULL_MOCK');
